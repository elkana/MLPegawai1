<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="uppercase" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'" />
	...
	<xsl:variable name="foto1">		
		<xsl:choose>
			<xsl:when test="exists(doc(concat('/personil_kemhan/',/personil_kemhan/file_foto_1)))">
				<xsl:value-of select="concat('/v1/documents?uri=', encode-for-uri(concat('/personil_kemhan/',/personil_kemhan/file_foto_1)))"/>
			</xsl:when>
			<xsl:otherwise>
